...
/// struct for typed errors of method [`get_post_reactions`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPostReactionsError {
...
pub async fn get_post_reactions(
    ...
    limit: Option<i32>,
    cursor: Option<&str>,
) -> Result<models::GetPostReactions200Response, Error<GetPostReactionsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_post_id = post_id;
    let p_query_account_id = account_id;
    let p_query_limit = limit;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetPostReactions200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetPostReactions200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetPostReactionsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
